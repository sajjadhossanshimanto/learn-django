...
class ManagerDashboardView(UserPassesTestMixin, View):
...
class UpdateTaskView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def post(self, request, id):
        ...
        task_detail_form = TaskDetailModelForm(request.POST, request.FILES, instance=getattr(task, 'details', None))
        if task_form.is_valid() and task_detail_form.is_valid():
            task = task_form.save()
            task_detail = task_detail_form.save(commit=False)
            task_detail.task = task
...
class TaskDetailsView(LoginRequiredMixin, PermissionRequiredMixin, View):
    permission_required = 'tasks.view_task'
    login_url = 'no-permission'
